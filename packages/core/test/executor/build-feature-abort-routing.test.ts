...
//
// When `plan` emits a fail outcome (which the PiCodergenBackend produces on
// `<abort>…</abort>`), the run must terminate at `done` without forwarding to
// `implement_and_review`, `verify`, `update_docs`, `commit`, or `merge`. Before
// this contract existed, a no-op / blocked plan would silently pass through
// the full pipeline and burn tokens on downstream nodes that had nothing to
// act on.
//
// The test injects a handler stub that emits a fail outcome on `plan` and
// asserts no downstream node starts.
...
describe("build-feature.dot — early abort routing", () => {
  ...
  test("success path is unchanged — on `ok` outcomes the unconditional edges still fire", async () => {
    // We only need to prove that plan → implement_and_review fires when plan
    // returns success. We truncate the run at implement_and_review's first
    // iteration (which returns APPROVED) by having the verify stub abort —
    // so we don't have to simulate CI loops or goal-gate retries here.
    const sink = new InMemorySink();
    await execute({
      graph: graph(),
      sink,
    ...
    const ran = nodesThatRan(sink.snapshot());
    // The key claim: plan → implement_and_review fires exactly because the
    // conditional `outcome=fail` edge does NOT match a success outcome.
    expect(ran).toContain("plan");
    expect(ran).toContain("implement_and_review");
    expect(ran).not.toContain("explore"); // explore no longer exists
  });
});
