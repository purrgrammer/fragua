// The `fragua ci` exit-code map. `ciExitCode` is the single status+reason →
// code function the command exits through; these lock the contract a CI
// pipeline branches on. The per-reason maps are `Record<Union, number>`, so
// totality is enforced by the type system (a missing literal is a compile
// error) — this suite pins the actual codes and the cross-cutting invariants.

import { describe, expect, test } from "bun:test";
import type { HaltReason, PauseReason, QuarantineReason, RunStatus } from "@fragua/types";
import { CI_EXIT, ciExitCode, HALT_EXIT, PAUSE_EXIT, QUARANTINE_EXIT } from "../src/ci-exit.ts";

describe("ciExitCode — stop-state + reason → exit code", () => {
  test("a clean exit is the only zero", () => {
    expect(ciExitCode("completed")).toBe(0);
    expect(CI_EXIT.ok).toBe(0);
    ...
    for (const s of nonZero) expect(ciExitCode(s)).not.toBe(0);
  });

  test("cancellation uses the SIGINT convention", () => {
    expect(ciExitCode("cancelled")).toBe(130);
  });
  ...
  test("one code per HaltReason (band 10s), all distinct", () => {
    ...
    for (const r of reasons) expect(ciExitCode("halted", { halt: r })).toBe(HALT_EXIT[r]);
  ...
  test("one code per operator PauseReason (band 30s), all distinct", () => {
    ...
    for (const r of operator) expect(ciExitCode("paused", { pause: r })).toBe(PAUSE_EXIT[r]);
  ...
  test("auto-wake PauseReasons are never a stop — they map to internal", () => {
    ...
    for (const r of ["provider_retry", "handler_retry", "timeout_retry"] as PauseReason[]) {
      expect(PAUSE_EXIT[r]).toBe(CI_EXIT.internal);
    }
  });

  test("one code per QuarantineReason (band 50s), all distinct", () => {
    const reasons: QuarantineReason[] = ["orphan_side_effect", "other"];
    for (const r of reasons) expect(ciExitCode("quarantined", { quarantine: r })).toBe(QUARANTINE_EXIT[r]);
    expect(new Set(reasons.map((r) => QUARANTINE_EXIT[r])).size).toBe(reasons.length);
  });

  test("paused_human is a single code (no reason enum)", () => {
    expect(ciExitCode("paused_human")).toBe(CI_EXIT.needsHuman);
  });

  test("a missing reason falls back to the band's generic code", () => {
    expect(ciExitCode("halted")).toBe(HALT_EXIT.error);
    expect(ciExitCode("paused")).toBe(PAUSE_EXIT.operator);
    expect(ciExitCode("quarantined")).toBe(QUARANTINE_EXIT.other);
  });

  test("a non-terminal status reached as a stop-state is the internal-error code", () => {
    expect(ciExitCode("paused_auto")).toBe(CI_EXIT.internal);
    expect(ciExitCode("queued")).toBe(CI_EXIT.internal);
    expect(ciExitCode("running")).toBe(CI_EXIT.internal);
  });

  test("the bands don't collide across reason kinds", () => {
    const all = [
      ...Object.values(HALT_EXIT),
      ...Object.values(PAUSE_EXIT).filter((c) => c !== CI_EXIT.internal),
