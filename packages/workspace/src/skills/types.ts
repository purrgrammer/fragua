// Skill metadata types live in @fragua/types so non-workspace packages
// (web UI, server, future store rows) can reference them without
// pulling the workspace runtime in. Re-exported here so existing
// `import { Skill } from "@fragua/workspace"` callers still compile.

export type { Skill, SkillCatalogRecord, SkillScope, SkillsConfig } from "@fragua/types";
