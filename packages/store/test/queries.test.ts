...
describe("run_state queries — conversation kind", () => {
  test("selectRunStateRow exposes kind + parent fields for a conversation run", async () => {
    const store = freshStore();
    const wf = await seedWorkflow(store);
    store.enqueueRun({ runId: "parent-1", workflowSha: wf, cwd: "/proj" });
    ...
    const child = store.getState("child-1");
    ...
    expect(child!.routing["input"]).toBe("do the thing");

    const parent = store.getState("parent-1");
    expect(parent!.kind).toBe("workflow");
    expect(parent!.workflowSha).toBe(wf);
    expect(parent!.parentRunId).toBeNull();
    store.close();
  });

  test("listChildRunIds returns child run ids for a parent, oldest-first", async () => {
    const store = freshStore();
    const wf = await seedWorkflow(store);
    store.enqueueRun({ runId: "p", workflowSha: wf });
    store.enqueueConversation({
      runId: "c1",
      parentRunId: "p",
      parentNodeId: "plan",
    ...
    expect(store.listChildRunIds("p")).toEqual(["c1", "c2"]);
    expect(store.listChildRunIds("unknown")).toEqual([]);
    store.close();
  });

  test("insertRunState (workflow path) requires a non-null workflow_sha; conversation rows accept NULL", async () => {
    const store = freshStore();
    const wf = await seedWorkflow(store);
    // Workflow path enforces the FK→workflows(sha) at the writer.
    expect(() => store.enqueueRun({ runId: "r-bad", workflowSha: "missing-sha" })).toThrow();
    // Conversation row inserts with workflow_sha=NULL by construction.
    store.enqueueRun({ runId: "p", workflowSha: wf });
    store.enqueueConversation({
      runId: "c",
      parentRunId: "p",
    ...
    expect(store.getState("c")?.workflowSha).toBeNull();
    store.close();
  });

  test("existing list-runs queries surface both workflow and conversation rows", async () => {
    const store = freshStore();
    const wf = await seedWorkflow(store);
    store.enqueueRun({ runId: "p", workflowSha: wf });
    store.enqueueConversation({
