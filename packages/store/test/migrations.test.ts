...
describe("migrate — v6 → v7 drops conversation-run scaffolding", () => {
  ...
  test("step migration is atomic: a foreign_key_check violation rolls back the schema_version bump", () => {
    // Regression for the harness-boot bug: foreign_key_check used to
    // run AFTER the transaction committed, so a v7 → v8 step with FK
    // violations advanced schema_version to 8 and only THEN threw.
    // The DB was wedged at v8 with violations until the operator
    // wiped or vacuumed manually.
    //
    // We reproduce a non-orphan-style FK violation that the v8
    // vacuum does NOT clean (an artifact pointing to a missing blob)
    // and assert migrate() throws AND schema_version stays at 7.
    const db = freshDb();
    db.exec("PRAGMA foreign_keys = OFF");
    db.exec(`
      CREATE TABLE schema_version (id INTEGER PRIMARY KEY CHECK (id = 1), version INTEGER NOT NULL) STRICT;
      ...
      INSERT INTO run_state (run_id, version, status, current_node, workflow_sha, schema_version,
        routing, metrics, priority, enqueued_at, ready_at, updated_at)
        VALUES ('r-real', 1, 'queued', NULL, 'wf-1', 7, '{}', '{}', 0, 0, 0, 0);
      -- Artifact pointing to a blob that doesn't exist. Vacuum's
      -- run_id-orphan sweep won't catch this; vacuum's blob-orphan
      -- sweep deletes blobs NOT in artifacts (the wrong direction).
      -- So this synthetic violation survives the vacuum and
      -- foreign_key_check fires.
      INSERT INTO artifacts (run_id, node_id, iteration, key, blob_sha, created_at)
        VALUES ('r-real', 'n', 0, 'k', 'sha-missing', 0);
    `);

    expect(() => migrate(db)).toThrow(/FK violations/);

    // Critical assertion: schema_version did NOT advance. Without
    // the atomicity fix, this row would read 8.
    const row = db.query<{ version: number }, []>("SELECT version FROM schema_version WHERE id = 1").get();
