...
describe("resume integration — activeMs, dispatch_started, crash recovery", () => {
  ...
  test("nodeOutputs captured before pause are still substituted into the next node's prompt after resume", async () => {
    // The bug fix that surfaced this test: $<nodeId>.output substitution
    // silently returned "" because no caller ever populated nodeOutputs.
    // Pause + resume amplifies the contract: after a wait.human gate the
    // daemon can be a fresh process, with no in-memory cache of prior
    // node outputs. The fold that materialises nodeOutputs at dispatch
    // time MUST recover from disk (events + artifacts only) so the
    // resumed B-node sees A's captured output verbatim.
    const dot = `digraph { start [shape=Mdiamond]; b [shape=box]; start -> b; b -> __end__ }`;
    const r = makeRig(dot);

    // Node `start`: write the canonical "output" artifact (mirrors what
    // handler-bridge does after a real codergen run) and transition to b
    // with an outputRef pointing at it.
    r.dispatcher.register(r.workflowSha, "start", {
      kind: "start",
      sideEffect: "none",
      maxMs: 200,
      handler: async (ctx) => {
        const ref = ctx.artifacts.put("output", "RESUMED_OK: A spoke", "text/plain", { replace: true });
        return { kind: "transition", nextNode: "b", tokens: 0, costUsd: 0, outputRef: ref };
      },
    });

    enqueue(r, "rs-no-1");

    // Run far enough to dispatch A and have currentNode advance to b,
    // then STOP. Turn 1 emits fact.run_started + advances, turn 2
    // dispatches the start handler. We bound at 2 so B is queued for
    // dispatch but never fires.
    {
      const ac = new AbortController();
      r.store.claimNextRun(1);
      await runOne("rs-no-1", {
        store: r.store,
        dispatcher: r.dispatcher,
        registry: new AbortRegistry(),
        tools: new handler.InMemoryToolRegistry(),
        llmCall: async () => ({ content: "", tokens: 0, costUsd: 0, model: "stub" }),
        maxConcurrentRuns: 1,
        maxTurnsForTesting: 2,
        shutdownSignal: ac.signal,
      });
    }

    // After A: status is running with currentNode=b (queued for next
    // dispatch). Force a pause as if a gate fired: append
    // fact.run_paused_hitl to mirror what wait.human would emit.
    const sAfterA = r.store.getState("rs-no-1")!;
    expect(sAfterA.currentNode).toBe("b");
    r.store.appendFact(
      "rs-no-1",
      [{ type: "fact.run_paused_hitl", payload: { nodeId: "b", label: "wait", options: [] } }],
      sAfterA.version,
    );
    expect(r.store.getState("rs-no-1")!.status).toBe("paused_hitl");

    // Simulate the daemon dying: close the store. The next handle is a
    // fresh process — no in-memory state, only what's on disk.
    r.cleanup();
    const store2 = reopenStore(r.dbPath);
    const dispatcher2 = new Dispatcher();

    // Register node B on the FRESH dispatcher. Capture the substitution
    // map handed to it, then complete normally.
    let capturedNodeOutputs: ReadonlyMap<string, { output: string; success: boolean }> | undefined;
    dispatcher2.register(r.workflowSha, "b", {
      kind: "step",
      sideEffect: "none",
      maxMs: 200,
      handler: async (ctx) => {
        capturedNodeOutputs = ctx.nodeOutputs;
        return { kind: "transition", nextNode: "__end__", tokens: 0, costUsd: 0 };
      },
    });

    // Operator resumes via HITL. This is the path the bug must survive.
    store2.appendIntent("rs-no-1", { type: "intent.hitl_input", payload: { selected: "go" } });
    wakePending(store2);
    expect(store2.getState("rs-no-1")!.status).toBe("queued");

    await runUntilSettled(store2, dispatcher2, "rs-no-1");

    expect(store2.getState("rs-no-1")!.status).toBe("completed");
    expect(capturedNodeOutputs).toBeDefined();
    expect(capturedNodeOutputs?.get("start")?.output).toBe("RESUMED_OK: A spoke");
    expect(capturedNodeOutputs?.get("start")?.success).toBe(true);
    store2.close();
  });
