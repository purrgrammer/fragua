...
describe("auto-dispatcher → fan_in LLM wiring", () => {
  function fanInDot(promptAttr: string): string {
    return `
      digraph {
        start  [shape=Mdiamond]
        ...
        a      [prompt="branch a"]
        b      [prompt="branch b"]
        join   [shape=tripleoctagon${promptAttr}]
        done   [shape=Msquare]
  ...
  test("tripleoctagon with prompt= and no delegate produces a halt spec with explanatory detail", async () => {
    const store = new SqliteStore({ path: ":memory:" });
    store.saveWorkflow("sha", "t", fanInDot(`, prompt="pick the best"`));
    const dispatcher = new Dispatcher();
    dispatcher.setResolver(autoDispatcherResolver({ store }));
    const spec = dispatcher.get("sha", "join");
    expect(spec.kind).toBe("parallel.fan_in");
    // Calling the handler with empty routing should still hit the malformed-spec path first.
    const result = await spec.handler({} as Parameters<typeof spec.handler>[0]);
    expect(result.kind).toBe("halt");
    if (result.kind === "halt") {
      expect(result.detail).toMatch(/prompt= set but no LLM delegate configured/);
    }
    store.close();
  });

  test("tripleoctagon without prompt= uses heuristic and ignores absent delegate", async () => {
    const store = new SqliteStore({ path: ":memory:" });
    store.saveWorkflow("sha", "t", fanInDot(""));
    const dispatcher = new Dispatcher();
    dispatcher.setResolver(autoDispatcherResolver({ store }));
    const spec = dispatcher.get("sha", "join");
    expect(spec.kind).toBe("parallel.fan_in");
    // Handler should be the heuristic spec (no delegate needed).
    expect(typeof spec.handler).toBe("function");
    store.close();
  });

  test("tripleoctagon with prompt= and a delegate is wired to the delegate", async () => {
    const store = new SqliteStore({ path: ":memory:" });
    store.saveWorkflow("sha", "t", fanInDot(`, prompt="pick the most critical"`));
    ...
    // Invoke with a routing that carries branch results.
    const routing: Record<string, unknown> = {
      "parallel.fanout.results": [
        { branchId: "a", status: "success" },
        { branchId: "b", status: "success" },
      ],
    };
    const result = await spec.handler({
      ...
      emit: () => {},
    } as unknown as Parameters<typeof spec.handler>[0]);

    expect(result.kind).toBe("transition");
    expect(capturedInputs).toHaveLength(1);
    expect(capturedInputs[0]!.prompt).toBe("pick the most critical");
    expect(capturedInputs[0]!.candidates).toHaveLength(2);
    store.close();
  });
});
