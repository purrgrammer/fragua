...
Vite + React + Tailwind client for the swarm HTTP/SSE server.
...
# Dev server (expects @swarm/server on :3000 — see below)
bun run --filter='@swarm/web' dev          # Vite on http://localhost:5173

# Production build → packages/web/dist
bun run --filter='@swarm/web' build

# Typecheck / tests
bun run --filter='@swarm/web' typecheck
bun run --filter='@swarm/web' test
```
...
(future) production builds served by `swarm serve`.
