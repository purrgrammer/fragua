...
| `intent.steering_requested` | `queued` / `running` / `paused_hitl` / `paused` / `paused_provider_retry` / `paused_retry` | concat `text` into `steering` (commit-order, `\n`-separated) | `quarantined` / terminal → drop `wrong_state` |
| `intent.hitl_input` | `queued` / `running` (post-wake) / `paused_hitl` / `paused` / `paused_provider_retry` / `paused_retry` | set `hitlInput` (last-wins on multiple) | `quarantined` / terminal → drop `wrong_state` |
