...
- **A pre-claim steer is threaded through routing.** The fold populates `decision.steering` from `intent.steering_requested` as normal, and the executor stashes that text into `run_state.routing.internal.pending_steer` on the run-start routing patch. Every pre-claim intent — the steer included, plus the synthetic `intent.run_enqueued` marker and any co-arriving `budget_adjusted` — advances `last_applied_seq`, so the supervisor never mistakes one for a fresh operator intent mid-handler and never re-folds it. (`last_applied_seq` is a single watermark, so the steer's seq cannot be held back while a higher-seq non-steer intent advances — advancing past the later intent would bury the steer below the watermark.) On each subsequent dispatch the executor surfaces the pending steer as `ctx.steering` (merged with any steer the fold consumed from the intent log), and the llm handler bridge injects it at the head of the first user turn. The transition planner clears `internal.pending_steer` once an llm step consumes it (completes with a success outcome); a `tool`/`human` first node ignores `ctx.steering` and leaves the key in place, so the steer carries forward to the first llm step. This mirrors the `internal.operator_notes` gate-note path. A `parallel` first node delivers the pending steer to every branch handler before the planner's per-branch clear fires, so no branch dispatches blind. A steer that folds *mid-run* while the frontier reaches a parallel node reaches every branch the same way: when the frontier is already seeded the executor threads that turn's `decision.steering` into each branch's `ctx.steering` directly; when the steer folds on the seed superstep (its seq consumed by `takeFold` before any branch dispatches) `runFanout` stashes the text into `internal.pending_steer` on the `fact.fanout_started` commit, exactly as run-start does, so the next superstep's branch dispatch surfaces it rather than dropping it. When a pre-claim pause co-arrives with the steer the fold returns R3 (`shouldPauseAfterDispatch`); because run-start advances past both intents, the executor stashes an `internal.pause_after_dispatch` marker on the run-start routing patch and the first running-turn dispatch consults it exactly as later turns consult `decision.shouldPauseAfterDispatch`. The transition planner both effects the pause (its R3 swap of the success continuation for `fact.run_paused`) and clears the marker (`buildRoutingPatch`) once the pause lands, so the run does not re-pause on resume. A `parallel` first node honours the same marker at its join barrier: the fan-out join is that path's success continuation, so when the marker is live `runFanout` emits `fact.run_paused` in place of `fact.fanout_joined` and clears the marker in the same commit — resume re-drains to the join and advances past it cleanly.

Mid-flight steers (issued while a handler is running) ride pi-agent-core's steering queue via the supervisor's `onSteer` instead; the `ctx.steering` path is the pre-dispatch delivery mechanism. A mid-flight steer **broadcasts to every in-flight LLM branch** of the run — under a `parallel` fan-out the steer reaches all concurrent branches, not just whichever one registered last (they are independent sub-pipelines, so the steer applies to the run). Delivery is recorded as `fact.steering_applied` (`disposition: delivered|buffered`, `targets`, `intentSeq`), so `fragua runs steer` and `runs events` can report whether the model actually saw the text rather than only that the intent was appended.
