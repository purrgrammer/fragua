...
| `intent.steering_requested` | `queued` / `running` / `paused_hitl` / `paused` | concat `text` into `steering` (commit-order, `\n`-separated) | `quarantined` / terminal → drop `wrong_state` |
| `intent.hitl_input` | `queued` / `running` (post-wake) / `paused_hitl` / `paused` | set `hitlInput` (last-wins on multiple) | `quarantined` / terminal → drop `wrong_state` |
