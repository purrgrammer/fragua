# swarm — DB-Backed Rearchitecture Plan

> Greenfield migration from file-based coordination (events.jsonl + checkpoint.json + control.jsonl + fs.watch) to a single SQLite-backed event store with projection-in-txn, intent/fact split, hard-abort semantics, and content-addressed blob storage.
>
> Companion to `SPEC.md`. Supersedes the file-based sections of `ARCHITECTURE.md`, `daemon.md`, `run-control.md`, and `events.md` once Phase 2 lands.
>
> **Revision 2.** Incorporates adversarial review: quarantine of orphaned side effects, loop-scoped artifact PK, deleted IPC module (DB polling instead), startup sweep, counter-based seq assignment, rowid `blobs`, folded `jobs`→`run_state`, `STRICT` tables, `ON DELETE CASCADE`, and `ready_at`-based queue fairness.
