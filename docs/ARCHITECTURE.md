...
| `intent.accept_run` | — | Post-terminal: land the run's work on the operator's current branch — the daemon replays the run's commits onto HEAD (message + author preserved) and stages the uncommitted tail for the operator to commit. Folded into `fact.run_accepted`; inbox `pending → acted` |
