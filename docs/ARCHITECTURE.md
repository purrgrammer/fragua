...
- **Hard abort for all interrupts.** Pause, cancel, and steer all trip a single `AbortSignal`. Handlers unwind, emit `fact.node_aborted` with partial metrics, executor re-enters (or halts) based on new state. A steer is the one exception to the trip: it rides pi-agent-core's steering queue instead and **broadcasts to every in-flight LLM branch** of the run (fan-out branches are independent sub-pipelines, so the steer applies to the run, not to whichever branch registered last); delivery is recorded as `fact.steering_applied`.
...
All tables are `STRICT`. The append-mostly per-run tables (`events`, `messages`, `artifacts`) and the global content-addressed `blobs` table additionally use `WITHOUT ROWID` for compact PK-clustered storage; the lifecycle and singleton tables (`schema_version`, `workflows`, `run_state`, `daemon_lock`, `daemon_events`) use the default rowid layout (`daemon_events` in particular relies on `INTEGER PRIMARY KEY AUTOINCREMENT`, which is incompatible with `WITHOUT ROWID`). (`blobs` is a global CAS keyed by `sha256` — no `run_id` FK, no cascade on run deletion; `artifacts` FKs into it and cascades; orphan blob files are reclaimed by `gcBlobs`.) Every table is narrow — the only "big" data (artifact content) lives on the filesystem under `blobsDir`, keyed by sha256. Per-run tables cascade on run deletion.
...
| `provider_credentials` | Built-in pi-ai provider credentials plus the `typesafe` judge provider, one row per provider id. | PK `provider`; `payload` is the full `AuthCredential` JSON; `kind` denormalises `payload.type` for post-mortem SELECTs. No indexes (<20 rows; PK is the only access pattern). The agent's `SqliteAuthStorageBackend` rebuilds the in-memory blob on read and applies a returned `next` blob by full-replace. |
...
| `fact.steering_applied` | `intentSeq`, `disposition: 'delivered'\|'buffered'`, `targets: {nodeId, iteration}[]` | Observability-only record of a mid-flight steer the supervisor forwarded: `delivered` broadcast into every in-flight LLM branch (`targets` lists them), or `buffered` when the handler was dispatched but its agent had not registered yet. That race is the only thing the in-process buffer covers — a steer issued while nothing is dispatched never reaches the steering registry at all, because the intent fold stashes it in `routing.internal.pending_steer` and the executor surfaces it as `ctx.steering` on the next dispatch (see `docs/intent-fold.md`). The buffer is therefore cleared when the run's live-agent set EMPTIES, not when it is first drained: under fan-out the window holds N racing branches, and draining into whichever one registers first would hand the steer to one branch and drop it for the rest. Projection-neutral (reducer no-op); joins to the originating `intent.steering_requested` via `intentSeq` so `fragua runs steer` / `runs events` can report whether the model saw the text |
...
Anything emitted via `ctx.emit` from a handler — `agent.message_start/end`, `llm.text_delta`, `llm.thinking_delta`, `llm.toolcall_delta`, `cost.recorded`, `tool.execution_start/end`, `budget.warn` / `budget.stop`, etc. — plus the executor's own `intent.dropped` (an intent it could not apply, see `docs/intent-fold.md`) and the judge handler's `judge.requested` / `judge.answered`. Best-effort streaming telemetry, not transactional bundle: no version bump, no decision logic reads them, consumers are SSE tails and projections. Written via `appendObservabilityEvents` (`packages/store/src/store.ts`): **no OCC** and **no reducer fold** (zero `run_state` projection impact), but they draw from the same per-run `seq` counter (`bumpRunSeq`) as facts, so they land in the same `seq` space and stay causally ordered against facts. An event whose payload exceeds the 4 KB cap is **truncated to a routing-preserving marker (`nodeId`, `iteration`) and logged, not rejected** — one oversize event never tanks the rest of the batch (full llm-turn content already lives in the `messages` table).
...
`llm.start.skills[]` carries one `SkillCatalogRecord` (see `packages/types/src/skills.ts`) per skill the model saw on this call. Each record includes `name`, `location`, `sha256`, `bytes`, `scope`, `source_dir`, optional `compatibility`, and — for `scope === "project"` — `project_cwd` so replay can correlate which project's skills were active for this run after per-run filtering at llm dispatch. When a run executes in a worktree (exec cwd ≠ project root), project-scope records are re-anchored from the discovery checkout to the run's own tree (`reanchorSkillsToRunTree`): `location`/`skill_dir`/`source_dir`/`project_cwd` point at the worktree copy of the same files when it exists there, so the advertised `<location>` — and every bundled resource path (`references/`, `scripts/`, `assets/`) the agent resolves against it — stays inside the run's cwd and passes the environment's path gate. A stale worktree that predates a skill keeps the discovery path and is still refused by the gate.
...
It covers the `daemon_events` append/read, the `daemon_lock` lifecycle (`acquire` / `forceAcquire` TTL-reclaim / `heartbeat` / `release` / `forceDeleteDaemonLock` / `evictDaemonLockIfStale` / `currentDaemonLock`), and schedule CRUD + the two daemon-side advancers (`recordScheduleFire`, `recordScheduleSkipped`). See `packages/store/src/types.ts`. `forceDeleteDaemonLock()` is the low-level eviction primitive — a single `DELETE FROM daemon_lock WHERE id = 1` that drops whatever lock row exists in one transaction, taking no pid; the server reaper calls it after its own TTL check. `evictDaemonLockIfStale({ ttlMs, isHolderAlive? })` wraps it with the gate: it deletes the row only when the heartbeat is past `ttlMs` OR the supplied liveness probe reports the holder dead, runs `startupSweep` first (crediting the dead heartbeat as `priorHeartbeatAt`), and never evicts a live daemon — the harness supervisor calls it (with a `kill(pid, 0)` probe) so a still-live orphan from a SIGKILLed sibling harness keeps its lock (SPEC I4).
...
- **`transition`** — the success path: optional `nextNode` (omit to let edge selection decide), `outcomeStatus` (`success`/`fail`/`retry`), an llm-set `route` on routing nodes, a single-line `failureReason` (surfaces as `fact.run_terminated{errored}.detail` on `fail → __end__`), the token/cost metrics with the optional four-bucket cost + token splits and `modelName`, and optional `outputs` (structured values from `emit_output` on llm steps or the `$FRAGUA_OUTPUT` file on tool steps).
...
- **`pause_provider`** — a recoverable provider transport failure carrying `httpStatus` (null on pre-response network failures), `provider`, `errorMessage`, and an optional `retryAfterMs` (provider `Retry-After`, honoured exactly when set, otherwise equal-jitter exponential).
