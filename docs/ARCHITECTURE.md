...
| `mcp_oauth` | Remote (HTTP) MCP server OAuth state (experimental). Secret-bearing; excluded from run bundles exactly like `provider_credentials`. | PK `url`; `payload` is an opaque JSON blob (OAuth client registration + token set) — the store round-trips it without modelling its internal shape. `json_valid` CHECK on `payload`. No indexes — PK on `url` is the only access pattern. |
...
Anything emitted via `ctx.emit` from a handler — `agent.message_start/end`, `llm.text_delta`, `llm.thinking_delta`, `llm.toolcall_delta`, `cost.recorded`, `tool.execution_start/end`, `budget.warn` / `budget.stop`, etc. Best-effort streaming telemetry, not transactional bundle: no version bump, no decision logic reads them, consumers are SSE tails and projections. Events land in the same `seq` space as facts.

`intent.dropped` is **not** a `ctx.emit` event — it is emitted directly by the executor via `appendObservabilityEvents` (see `packages/daemon/src/executor.ts`), not by a handler. One entry fires per intent the fold decided not to effect, carrying `{ originalSeq, originalType, reason }`. It rides the same per-run `seq` space. See `docs/intent-fold.md` for the drop-reason taxonomy (`wrong_state` / `superseded_by_cancel` / `later_input_won` / `already_paused`).
