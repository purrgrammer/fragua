...
- **Context-management hardening (multi-wave).** Drive `docs/SPEC.md` §3.3 + §3.5 to actually-implemented status and close observed gaps. Tracked under `tasks/p6/NN-context-mgmt-waveN.md`.
  ...
  - Wave 2 — Make declared knobs work. `MessageStore` in `@swarm/agent` so `fidelity=full` actually restores prior turns (pi-agent-core's `sessionId` is only a provider-cache hint, not a message-restore). Fidelity seeds for `truncate` / `compact` / `summary:low` (deterministic); `summary:medium` / `summary:high` soft-warn and fall back until the summariser backend lands in Wave 2b. Node-level `context = "fresh"` gate, per-node `system_prompt` override, two-phase `fallback_retry_target` (fresh budget after primary exhausts). Cache-aware `sessionId` bucketing per fidelity. **Landed.**
  - Wave 2b — Summariser backend + async pipeline auto-title. `SummariserBackend` port in `@swarm/core`, `PiSummariserBackend` default impl in `@swarm/agent` using pi-ai's `completeSimple`. `fidelity=summary:medium|high` now makes a real LLM call to the summariser for a narrative tail; failures fall back to the deterministic `summary:low` template with a soft warning. `execute()` kicks off an async title summariser over `$ARGUMENTS` at pipeline start (configurable via graph `auto_title` / CLI `--no-auto-title`), emitting `pipeline.title_generated` under synthetic node `__summary.title`. Each summariser call records cost under its synthetic `node_id` (`__summary.title` / `__summary.<caller>`) so drilldown + cost buckets stay clean. Server + web surface `title` / `input` on the pipeline model. Retrofit script at `scripts/backfill-titles.ts`. **Landed.**
  - Wave 3 — Close test-coverage gaps. Integration tests for thread_id session reuse, context_files end-to-end, retry context propagation, goal-gate × abort interaction, parallel-branch in-progress isolation, steering targeting + robustness, substitution property-based fuzz, and the degradeOnResume rule. **Landed** (689 → 715 tests; +26 integration + property cases across 8 new files).
  - Wave 4 — Budget scaffolding. Per-node (`max_cost_usd`, `max_tokens`) + per-run (`budget_usd`, `budget_tokens`) ceilings; `BudgetLedger` pure reducer + sink-wrapping integration; `budget.warn` (≥80 %) / `budget.stop` (≥100 %) events under synthetic node `__budget`; `budget_policy` (`"stop"` default, `"warn"` opt-in); backend pre-flight returning non-retryable on breach; `llm.start.budget` carries real cumulative values. **Landed** (716 → 731 tests; +15 unit + integration across 2 new files).
  - Wave 5 — Server + UI introspection. `GET /pipelines/:runId/steps` returns a `StepSnapshot[]` reconstructed server-side from the raw event stream (one per `llm.start`). Web `<StepInspector>` panel under PipelineDetail renders collapsible sections for prompt, system prompt, messages, tools, context files, settings, budget, cost. Introduced `EventSource` + `Projection<T>` + `projectRun` + `foldAll` + `migrateAllRuns` in `@swarm/events` as the read-side abstraction + sink-migration path; `StepSnapshot` and `stepsProjection` live in `@swarm/server/lib/steps.ts`. **Landed** (731 → 748 tests; +17 reducer + route + component).
  - Wave 6 — Wrap-up. Streaming summariser (`streamSimple` + new `summary.text_delta` event + TypeBox); checkpoint-resume end-to-end (`CheckpointStore` port + `JsonlCheckpointStore` adapter + `execute({ checkpointStore, resume: true })` hydrates state + `pi_sessions` + applies `degradeOnResume` to the first resumed node's fidelity); cleanup pass converts `/stats`, `/pipelines`, `/pipelines/:id` to consume the Wave-5 `Projection<T>` + `foldAll` + `projectRun` helpers; `summaryProjection` + `detailProjection(runId)` exported with stable cache keys. AGENTS.md ground rules now include **NO INLINE IMPORTS** (hoist all imports; no `await import(…)` inside functions). **Landed** (748 → 757 tests; +9 streaming reducer + checkpoint-resume + JsonlCheckpointStore).
...
- **`local:web_fetch`:** HTTP GET a URL with SSRF guard (rejects non-http(s), loopback, RFC1918, link-local), `Accept: text/markdown,...` preference, `format: auto` converts text/html → markdown via `turndown` (scripts/styles/nav stripped), `format: raw` returns untouched body; `max_bytes` cap (default 1 MB), 30 s timeout. Tool description nudges agents toward `/llms.txt` / `/llms-full.txt` well-known paths. **Landed.**
- **`local:apply_patch`:** OpenAI v4a envelope (`*** Begin Patch` / `*** Update|Add|Delete File` / `@@ anchor` / context + `-`/`+` lines / `*** End Patch`). Multi-file, multi-hunk in a single call — replaces long sequences of `read_file` / `write_file` / `edit_file` when a change spans many locations. Pre-validates the entire patch against current file state before any write, so a failing hunk never leaves partial state. Anchors disambiguate repeated blocks; without an anchor, hunks require a unique (context + remove) match. `*** Move File` deferred. **Landed.**
- **Skills (agentskills.io):** three-tier progressive disclosure with auto-discovery (`.swarm/skills`, `.agents/skills`, `.claude/skills` under project + user scopes). Tier-1 catalog prepended to the system prompt, tier-2 `local:load_skill` tool returns SKILL.md body, tier-3 resources via `local:read_file`. Config override via `skills.paths` in `.swarm/config.yaml`. Durable on `llm.start.skills[]` for replay drift detection. Server exposes `GET /skills` / `GET /skills/:name`; web UI has a browse surface under `/skills`. Subagents inherit the catalog but not activated bodies; `local:subagent preload_skills: […]` pre-activates in the child. **Landed.**
