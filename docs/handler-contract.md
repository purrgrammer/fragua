...
Inside a `parallel` region (§3.1.1) the note is addressed to the **region**: every branch entry receives the notes pending at turn start, no deeper sub-node does. The fan-out dispatch view deliberately folds only `internal.retry_count.*` from same-turn commits, so it cannot observe the clear a settled sibling wrote; delivery is scoped to branch entries instead. Without that scoping a correction aimed at a branch's first step is re-applied, with the same overriding framing, by the later step whose job was to judge that step's output.
