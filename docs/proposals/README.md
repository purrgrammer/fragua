...
| [Reject unbounded loops by construction](./bounded-loops.md) | specified | validator E032: every cycle (plain `next`/`on` edges, not just `retry:`) must carry a finite `max_retries` on a node in it, else reject at validate time. Closes the class behind the `work` ci↔fix budget-burn (`5f9a0356` fixed the one workflow) |
