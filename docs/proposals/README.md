...
| [`reactive-frontier.md`](reactive-frontier.md) | plan (ready-to-build) | Implementation gameplan: replace `runFanout`'s `Promise.all` superstep batch (head-of-line blocking — a hung branch dams finished siblings, confirmed in a live post-mortem) with a `Promise.race` commit-as-settled pool; + the two liveness gaps (per-branch watchdog, per-branch abort-loop). Pick up fresh. |
