...
| [`typed-routing-struct.md`](typed-routing-struct.md) | draft (parked) | Typed wrapper struct for `run_state.routing` (option (c), reserved namespaces) replacing the unschematized `Record<string, unknown>`. Design sound; 5 open precision items (genesis-sanitize cut, `getInputs` guard parity, drop recursive `InputValue` tower, single-seam lint count, `@fragua/cli` blast radius). Pairs with the cap-as-tripwire decision. |
