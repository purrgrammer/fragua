...
| [`pi-085-auth-migration.md`](pi-085-auth-migration.md) | sketch | Migrating fragua off pi-ai's removed global OAuth registry (gone at 0.80.8; `/oauth` is type-only by 0.85.1). The registry is replaced by per-provider `ProviderAuth { apiKey?: ApiKeyAuth; oauth?: OAuthAuth }`, a `CredentialStore` port (structural match to fragua's `AuthStorage` lock), and `Models.getAuth(model)` as the single request-auth resolver. Covers the four call sites (`auth-storage.ts`, `model-registry.ts`, cli `providers.ts`, server `providers.ts`), confirms `mcp/oauth.ts` is a separate (MCP-client) subsystem and untouched, and lays out a 6-PR plan that keeps catalogue reads on `/compat` and only bumps the pin last. Unblocks the ≥0.85 bump the straight-to-0.85.1 attempt correctly aborted. Open: provider- vs model-scoped auth, `modifyModels` successor, ambient-credential policy. |
...
| [`tool-outputs.md`](tool-outputs.md) | draft | Typed `outputs:` on `tool` steps — the deferred follow-on 3 of `structured-outputs.md`. Emission through a `$FRAGUA_OUTPUT` scratch file allocated by a new `ExecutionEnvironment` capability and read back through a retained `O_CLOEXEC` fd, so a child that swaps a symlink/FIFO/device onto the path fails closed. Reuses the whole write-side spine — type grammar, outputs index, `fact.node_completed.payload.outputs`, blob spill, `${{ outputs.X.f }}`. No new fact type, no reducer change, no `EVENT_CONTRACT_VERSION` bump; `ir_version` bump + converter. Reverses the tool-production ban in SPEC §3.8. Unreviewed draft — see #80. |
