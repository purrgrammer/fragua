...
| [Workflow JSON-IR storage + version tracking](./workflow-ir-storage.md) | designed | store the parsed canonical JSON IR + an `ir_version` alongside the YAML `source` (rename of `dot_source`); parse-once dispatch + version gating. Adversarial finding: keep `sha = hash(source)` as the durable content address (run FKs + the immutable `fact.run_started{workflowSha}` log forbid re-hashing on the IR), treat the IR as a re-derivable cache. Closes #26 + the deferred `dot_source` rename |
