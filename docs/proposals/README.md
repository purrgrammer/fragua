...
| [Skills + agents UI + unified discovery](./skills-and-agents-ui.md) | designed | top-level `/skills` + `/agents` (list + detail), per-project nesting, read-only file-tree on skill detail with markdown render + image inline + hex-dump fallback; daemon discovery widens from one-cwd-at-boot to globals + every cwd in `store.listCwds()`, codergen filters per-run by `run.cwd`, auto-scan on first-sight for never-seen project cwds; new `Skill.project_cwd` / `AgentDefinition.project_cwd` (and on `SkillCatalogRecord` — same-PR ARCH §3 update) |
