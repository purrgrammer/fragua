...
| Proposal | Maturity | Notes |
|---|---|---|
| [Worktrees](./worktrees.md) | designed | per-run worktree isolation + tree snapshots at every step/HITL/terminal boundary (Diff scrubber) + two synchronous post-run actions — **accept** (replay the run's commits onto your branch, stage the uncommitted tail for you to commit) / **discard** — on `refs/swarm/{snapshots,heads}/<runId>`; recoverable runs land in an operator **inbox**; clean runs disappear quietly. Replaces [worktree-design.md](./worktree-design.md) and [worktree-snapshots.md](./worktree-snapshots.md) |
| [Reject unbounded loops by construction](./bounded-loops.md) | specified | a workflow cycle with no iteration cap is rejected at validation rather than relying on a runtime budget/time backstop; outstanding: the validator rule + authoring-error wiring |
...
| [Resume continuity for fresh-thread nodes](./resume-continuity.md) | designed | budget-pause resume restarts a fresh-thread node (e.g. `fix`) from its prompt — `shouldHydrate/PersistFromStore` gate on thread membership, so a non-threaded node neither saves nor reloads its own in-flight transcript. Decouple resume continuity (per `(nodeId, iteration)`) from cross-node thread sharing via a node-local resume buffer |
...
| [Structured step outputs](./structured-outputs.md) | designed | one-directional typed data flow over the `script`/`llm`/`human` graph: per-step `outputs:` (mirroring `inputs:`, produced by both `script` and `llm`) + `${{ steps.X.outputs.f }}` substitution, enforced by a forced `emit_output` tool (runtime) + dominance check (validator). Adds equality-only `when:` fact-routing (reads `inputs ∪ dominating-outputs ∪ outcome`) alongside the kept `route` tool; collapses the data-plumbing half of shared-thread usage (`review::scope` emit block, `pr_*` body/PR# scraping) into bare-`script` action paths. Spine: predicates compute in steps, edges only match values; correctness is a property of the `llm`↔`script` topology. Carries the `tool`→`script` rename. Evolves AGENTS.md rule 13; the GHA-shaped substrate for repo-triggered runs |
