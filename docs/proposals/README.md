...
| [Provider credentials in the store](./provider-credentials-storage.md) | specified | move built-in pi-ai provider credentials out of `~/.swarm/auth.json` into a `provider_credentials` table; cuts `!cmd`/`env` resolution from the main path; custom providers stay on `models.json` until the follow-up below lands |
