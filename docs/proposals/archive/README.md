...
| [`tool-exec-variant.md`](tool-exec-variant.md) | **Superseded** — the `tool` node is being rethought from scratch; the argv `exec:` form + `idempotent:` marker are deferred into that redesign rather than added as a variant of the shell form (see [`embeddable-engine.md`](../embeddable-engine.md) §5.3). |
...
| [`reversible-migrations.md`](reversible-migrations.md) | **Shipped** (#17) — each `SCHEMA_MIGRATIONS` step gains an optional `down` inverse + `fragua db migrate --to <version>` walks down as well as up. Downgrade is run by the *newer* binary, backed up first, gated on reversibility + daemon-liveness; orthogonal to the `EVENT_CONTRACT_VERSION` resume gate. |
