...
- **`paused`** (extends attractor §3.1). Attractor models provider transport failures and budget overruns as either retryable-by-policy or terminal FAIL. Real operators need a recoverable state: pause the run with the transcript intact, fix the API key / quota / budget cap, then `intent.resume`. swarm collapses this into one operator-resumable status with a typed `reason` on `fact.run_paused` — twelve reasons across three projecting statuses (`paused` / `paused_auto` / `paused_hitl`). The operator-resumable family covers `operator`, `provider_error`, `payment_required`, `budget`, `max_retries`, `goal_gate`, `max_loops`, `abort_loop`, `provider_exhausted`. The auto-wake family (status `paused_auto`) covers `provider_retry`, `handler_retry`, `timeout_retry` — daemon timers re-queue these on `auto_resume_at`; operators may short-circuit via `intent.resume`. Cap-adjustment intents (`intent.budget_adjusted`, `intent.max_retries_adjusted`, `intent.goal_gate_adjusted`, `intent.max_loops_adjusted`) raise per-run ceilings before resume. Adding a new operator-fixable failure mode is one new reason — no schema or status churn. After this taxonomy, `HaltReason` keeps only the genuinely terminal: `schema_drift`, `error`, `occ_exhausted`, `aborted_exit`, `budget` (when `budget_policy="stop"`), `timeout_exhausted`. See `docs/proposals/recoverable-budget-pause.md`.
...
- **`abort` self-abort tool** — codergen agents signal "I cannot proceed" by calling the built-in `abort` tool with a `reason`. The tool is force-included on every codergen node (like `skill`); its description teaches the contract. `findAbortToolCall` scans the transcript for the call and translates it to `outcome.status="fail"` (`non_retryable`) so workflows route via `condition="outcome=fail"` edges. Not orchestration but contract between agent and codergen handler.
