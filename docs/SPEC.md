...
- **`<abort>` self-emit contract** — codergen agents signal "I cannot proceed" by emitting `<abort>reason</abort>` as the entire last non-empty line of their final message. Taught in the system prompt's `<protocol>` block (constant per call, cache-key-clean) and parsed by `parseAbortMarker`. Translates to `outcome.status="fail"` so workflows route via `condition="outcome=fail"` edges. Not orchestration but contract between agent and codergen handler.
