...
- `local:*` — built-in tools (read_file, write_file, bash, grep, glob, edit_file, apply_patch, web_fetch, **load_skill**)
...
**Read-side abstractions (Wave 5).** `@swarm/events` exports the inverse of `EventSink` — `EventSource` (`listRuns`, `readRun`) — plus a `Projection<T>` type (pure reducer Event[] → T), the `projectRun` + `foldAll` helpers, and a `MaterializedProjectionStore<T>` interface that future DB-backed adapters implement when they want to precompute projections. `migrateAllRuns(source, sink)` is the supported path for moving a run archive from one backing store to another (JSONL → Postgres, etc.). The server's `RunReader` port is interface-equivalent and kept as a compatibility alias (`runReaderFromSource` bridges between the two method names).

`GET /pipelines/:runId/steps` — per-step snapshot endpoint introduced in Wave 5. Returns `StepSnapshot[]` (see `packages/server/src/schemas.ts`): one entry per `llm.start`, with `prompt`, `systemPrompt`, `messages`, `contextFiles` (sha256 + bytes + truncated + status), `allowedTools` / `deniedTools`, `settings`, `iteration`, `budget`, `cost`, `finalText` (assembled from `llm.text_delta` events), and `durationMs`. Reconstructed server-side via `stepsProjection` (`STEPS_PROJECTION_KEY = "steps"` for cache adapters).
