...
- LLM-spawnable sub-agents via the `agent` tool (`defaultDisabled: true`, opt-in per node via `allowed_tools=…,agent`): a codergen iteration constructs an inline spec (prompt, optional system prompt override, tool pool, skills, max iterations) and the host enqueues an isolated child run with `kind='conversation'` rooted at the parent codergen iteration. The child sees only the spec's prompt; structurally cannot recursively spawn `agent`; full child transcript replayable from its own `run_state` row + event stream. Two observability-only fact types (`fact.subagent.spawned` / `fact.subagent.completed`) ride the parent's stream, and an orphan-child sweep cancels children whose parent went terminal while the daemon was down — see [`docs/proposals/agent-tool.md`](docs/proposals/agent-tool.md)
