...
| E028 | Node id `exit` is reserved for the graceful-halt sink — declare it as `type: exit` or rename the step. |
| E029 | Node id `start` is reserved for the synthesized entry node — rename the step. |
| E030 | `${{ inputs.x }}` references an input not declared in the workflow's `inputs:` block (scans `prompt` / `text` / `tool_command`). Add it to `inputs:` or fix the typo. |
