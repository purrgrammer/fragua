...
| `fact.run_paused` | `reason: "engine_incompatible"` | Run's pinned `schema_version` falls outside this daemon's `[supportedMin, supportedMax]` fold window. `pinnedVersion > supportedMax` (too new — downgraded binary or newer-producer import) heals once a capable daemon runs; `pinnedVersion < supportedMin` (too old) needs an operator rebuild or cancel. Both project to `paused` — operator-resumable, not a terminal halt. |
