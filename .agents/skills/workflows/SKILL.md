...
**Harness-level judge assists are opt-in, never a default.** `judge.skill-suggestion` (one call per llm step names the skill the prompt calls for) and `judge.tool-guard` (`flag` / `block` on bash / write / edit / MCP calls) live in `config.yaml`, off unless set — a workflow never turns them on and should not assume they are running.
