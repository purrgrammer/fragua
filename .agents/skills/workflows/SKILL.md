...
- `decide.route: {question: <choice>, min-confidence?: 0..1, min-probability?: 0..1, below?: <route>}` + `routes:` — **`confidence` is how concentrated the distribution is, not the winner's probability**: options at 0.60 / 0.38 / 0.02 give confidence ≈ 0.39. Gate on `min-confidence` when "is the model sure which?" is the question; gate on `min-probability` (the chosen option's own probability) when you mean "the winner has p ≥ x". Declare both and `below` is taken when either fails; a floor needs `below` and vice versa. The chosen option is the route; under the floor, `below` is taken instead (point it at a `human` step to escalate, or at an option like `full` for "when torn, go deeper"). Options must match `routes:` (E047); W020 nags a ≥3-way choice with no floor.
...
**Give a `keep:` an uncertainty band.** A noul at 0.55 is not a no — TypeSafe's own cookbooks route 0.3–0.7 to a human rather than acting on it. `review: {present: {min: 0.3, max: 0.6}}` beside `keep: {present: 0.6}` sends those items to a third typed output, `${{ outputs.<judge>.review }}`, instead of `dropped`; the consumer settles them or lists them as uncertain. Without it a borderline item is discarded and nothing downstream can tell it apart from a confident no. `fragua judge calibrate <workflow>` reads past runs and reports how many of a gate's answers landed in that band, which is how you pick the numbers without a labelling set.

**Combine dimensions with `composite:`, not with a "how good overall?" question.** `composite: {quality: {correct: 3, clear: 1}}` is a weighted mean in [0, 1] of noul p(yes) and normalised score positions (a `choice` cannot carry weight — E052); it lands as a `number` output (`${{ outputs.j.quality }}`, `kept[].judge.quality`) and `decide.outcome` / `keep` threshold it like a noul (`{quality: 0.7}`). The raw answers stay, so re-weighting is a YAML edit, not a re-run. An "any serious violation" rule is still its own `max` threshold on the hazard noul — a mean hides it.
