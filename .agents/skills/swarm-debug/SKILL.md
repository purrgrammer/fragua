...
| `fact.run_halted` | `"occ_exhausted"` | Optimistic-concurrency retry budget exhausted on a single `(nodeId, iteration)`. Payload carries `occContext: { count, nodeId, iteration, lastVersion, attemptedFactType }` — read it for post-mortem instead of grepping `detail`. `attemptedFactType` names the fact whose append couldn't land; `lastVersion` is the run-state version observed before the storm. Source: `packages/types/src/swarm-events.ts` `fact.run_halted` payload. |
| `fact.run_halted` | `"provider_exhausted"` | The auto-retry chain capped out — 5 attempts or 5 cumulative minutes, whichever first — and the daemon stopped retrying. Operator decides: `intent.resume` to try once more, `intent.cancel`, or steer to a different provider. Walk the chain via the run's `fact.provider_retry_attempted` events plus the original `fact.run_paused{reason:"provider_error"}` for the full retry trail. |
| `fact.run_paused` | `reason: "operator"` | Operator hit Pause. Wake on `intent.resume`. |
| `fact.run_paused` | `reason: "provider_error"` | LLM provider returned a manual-class transport error (400/401/403/404/413/422). Payload: `{httpStatus, provider, errorMessage}`. Wake on `intent.resume` after fixing creds/request. |
| `fact.run_paused` | `reason: "payment_required"` | LLM provider returned 402. Operator tops up at the provider's console, then `intent.resume`. |
| `fact.run_paused` | `reason: "budget"` | Local budget cap hit (run or node). Payload: `{scope, metric, limit, actual}`. Operator raises via `POST /runs/:id/budget {scope, metric, newLimit}`, then `intent.resume`. (Default `budget_policy="pause"`; opt into `"stop"` for terminal-on-overspend.) |
